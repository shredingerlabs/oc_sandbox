...
- **Windows (from inside WSL)**: `OC Sandbox.lnk` in the current Windows user's Start Menu, created via `powershell.exe` (WScript.Shell COM). The `.lnk` target is `wsl.exe -e bash <install>/scripts/start-tui.sh` in the default distro; `IconLocation` points at the Windows-translated `.ico` path (`wslpath -w`).
